...
def main():
    ...
    if about_path.exists():
        ...
        header = "The following is the user profile and behavioral context, loaded automatically at session start:\n\n"
        if len(header) + len(content) > hook_budget(cwd):
            print("`_self/about.md` exceeds the hook budget and was not injected — read it now, then run /maintain option 5 to trim it.")
        else:
