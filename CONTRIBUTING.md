...
Framework files (`_scripts/`, `.claude/`, `_templates/`, `docs/`, `CONTRIBUTING.md`, etc.) travel to the upstream via `/contribute`. Two files from the second-brain-setup project also travel: `architecture.md` and `requirements.md`.
...
**Documentation consistency** — if you use Claude Code, run `/maintain` option 6 (Documentation maintenance) to check that the implementation, SE docs (`architecture.md`, `requirements.md`, `verification.md`), and README stay in sync.
